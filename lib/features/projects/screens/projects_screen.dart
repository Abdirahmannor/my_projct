...
class _ProjectsScreenState extends State<ProjectsScreen> {
  ...
  void initState() {
    ...
    checkedProjects = List.generate(
        context.read<ProjectProvider>().projects.length, (_) => false);
  }
                              ...
                              itemBuilder: (context, index) {
                                ...
                                return MouseRegion(
                                  ...
                                  child: ProjectGridItem(
                                    ...
                                    completedTasks: projects[index]
                                        ['completedTasks'],
                                    totalTasks: projects[index]['tasks'],
  ...
  void _updateStatistics(String timeframe) {
    ...
    print('Updating statistics for timeframe: $timeframe');
  }
  ...
  Widget _buildProjectsPieChart() {
    return Stack(
      ...
      children: [
        ...
        Text(
          '${((context.read<ProjectProvider>().projects.where((p) => p['status'] == 'completed').length / context.read<ProjectProvider>().projects.length) * 100).toStringAsFixed(0)}%',
          style: Theme.of(context).textTheme.bodySmall?.copyWith(
                fontWeight: FontWeight.bold,
              ),
        ),
      ...
      children: [
        ...
        Text(
          projectProvider.projects.isNotEmpty
              ? '${((projectProvider.projects.where((p) => p['status'] == 'completed').length / projectProvider.projects.length) * 100).toStringAsFixed(0)}%'
              : '0%',
          style: Theme.of(context).textTheme.bodySmall?.copyWith(
                fontWeight: FontWeight.bold,
                fontSize: 10,
  ...
  void _handleDelete(int index) async {
    final projectProvider = context.read<ProjectProvider>();
    final projectName = projectProvider.projects[index]['name'];
    ...
    if (confirm == true) {
      ...
      setState(() {
        checkedProjects =
            List.generate(projectProvider.projects.length, (_) => false);
      });
  ...
  void _handleEdit(int index) async {
    ...
    if (result != null) {
      projectProvider.updateProject(index, result);
      _showSuccessMessage(
  ...
  void _handleComplete(int index) async {
    final projectProvider = context.read<ProjectProvider>();
    final projectName = projectProvider.projects[index]['name'];
    ...
    final bool? confirm = await showDialog(
      ...
      builder: (context) => AlertDialog(
        ...
        content: Text(
          'Are you sure you want to mark "$projectName" as complete?',
        ),
        actions: [
          TextButton(
            onPressed: () => Navigator.pop(context, false),
    ...
    if (confirm == true) {
      ...
      _showSuccessMessage(
        message: 'Project "$projectName" has been marked as complete',
